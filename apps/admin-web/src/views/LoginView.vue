...
async function handleSubmit(): Promise<void> {
  ...
  loading.value = true
  try {
    // auth store login 已校验 role=admin，非管理员抛 2003
    await authStore.login(form)
    ElMessage.success('登录成功')
    // 优先回跳 redirect 参数指向的原页面，否则回 dashboard
    const redirect = (route.query.redirect as string) || '/'
    router.replace(redirect)
  } catch (err) {
    // 3002 防枚举：邮箱或密码错误；2003 非管理员；其他业务码直接展示 message
    ...
    <div class="w-full max-w-md">
      <div class="rounded-2xl bg-white p-8 shadow-sm ring-1 ring-gray-100">
        <header class="mb-6 text-center">
          ...
          <p class="mt-1 text-sm text-gray-500">IELTS Speaking 管理平台</p>
        </header>
